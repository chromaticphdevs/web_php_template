    ...
    if(isSubmitted()) {
        ...
        if(!empty($post['btn_register'])) {
            ...
            if(!$user) {
                ...
                $user = $serviceAccount->startAuth($userId);

                if($user) {
                    Flash::set("Welcome to our platform {$user['memberfname']}");
                    return redirect(_route('user_dashboard'));
                } else {
                    Flash::set("There's been an issue to this page, please report to administrator, try to login again.");
